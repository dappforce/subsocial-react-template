...
const FollowersBlock: FC<FollowersBlockProps> = ({
    ...
    const classname = className ? `${className} ${styles.followers}` : styles.followers
    const {isVisible, toggleModal} = useModal()
    const [postCount, postLabel] = pluralize(posts, 'Post', 'Posts').split(' ')
    ...
    return (
        <div className={classname}>
            ...
            <button><span className={styles.bold}>{transformCount(+postCount)}</span> {postLabel}</button>
