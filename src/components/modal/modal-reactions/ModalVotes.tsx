...
const loadSuggestedReactionsIds = async (api: FlatSubsocialApi, postId: PostId) => {
    const method = await api.subsocial.substrate.getPalletQuery('reactions')
    const ids = await method.reactionIdsByPostId(postId)
    return await api.subsocial.substrate.findReactions(ids as unknown as ReactionId[])
}

const getReactionsIdsByPage = (ids: Reaction[], size: number, page: number) => getPageOfIds(ids, {page, size})

function isUpvote(reaction: ReactionType): boolean {
    return reaction && reaction?.kind as unknown as string === 'Upvote'
}

const loadMoreReactionsFn = async (loadMoreValues: any & {api: FlatSubsocialApi}) => {
    const {
        size,
        page,
        api,
    ...
    await dispatch(fetchProfiles({api, ids: reactionsIds.map(item => item.created.account.toString()), reload: false}))
...
const ModalVotes: FC<ModalVotesProps> = ({ postId }) => {
    ...
    useEffect(() => {
        loadSuggestedReactionsIds(api, postId).then(ids => {
            const [ upvotes, downvotes ] = partition(ids.map(item => ({
    ...
    const loadMoreNext = useCallback(async (page, size) => {
    ...
    return (
        <ModalReactionsLayout
            ...
            isTabs={true}
            tabs={tabs}
            dataSource={value === 'upvotes' ? upvotes.map(item => item?.account) : downvotes.map(item => item?.account)}
