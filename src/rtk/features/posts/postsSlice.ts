...
import { AccountId, PostId, PostWithSomeDetails, ProfileData, SpaceData } from '@subsocial/api/flat-subsocial/dto'
...
export const fetchPosts = createAsyncThunk<PostStruct[], FetchPostsArgs, ThunkApiConfig>(
    ...
    async (args, {getState, dispatch}) => {
        ...
        const posts = await api.findPostsWithAllDetails({ids: newIds as unknown as AnyPostId[], visibility})
        ...
        if (withOwner) {
            ...
            if (owner.length) {
                const ownerContent = owner.map(post => ({...post.owner.content, id: post.owner.struct.contentId}))
        ...
        if (withSpace) {
            ...
            const spacesContent = posts.map(post => ({...post.space.content, id: post.space.struct.contentId}))
        ...
        if (withContent) {
            const postsContent = posts.map(post => ({...post.post.content, id: post.post.struct.contentId}))
