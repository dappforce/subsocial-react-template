...
export const selectMyReactionsByPostIds: SelectManyFn<Args, ReactionStruct> = (state, {myAddress, ids: postIds}) => {
...
export const selectMyReactionByPostId = (state: RootState, {postId, myAddress}: PostIdAndMyAddress) => {
    return getFirstOrUndefined(selectMyReactionsByPostIds(state, {ids: [ postId ], myAddress}))
}
type FetchManyReactionsArgs = FetchManyArgs<Args>

type FetchManyResult = ReactionStruct[]

export const fetchMyReactionsByPostIds = createAsyncThunk<FetchManyResult, FetchManyReactionsArgs, ThunkApiConfig>(
    `${sliceName}/fetchMany`,
    async (args, {getState}): Promise<any> => {
        ...
        const entities = await api.subsocial.substrate.findReactions(reactionIds.filter(isDef))
